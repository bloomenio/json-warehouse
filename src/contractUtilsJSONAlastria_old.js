...
const gasLimit = process.env.ALASTRIA_GAS_LIMIT;
const GAS = gasLimit;
...
async function _createContainer(json, name) {
    ...
    let containers = await _getContainers();

console.log("test",containers[containers.length-1].add);
    let address = containers[containers.length-1].add;
